...
local Ignore2 = {
    (pcall(function() return workspace.Debris end) and workspace.Debris) or nil,
    (pcall(function() return workspace.Ray_Ignore end) and workspace.Ray_Ignore) or nil
}
local Multipliers = {
    Head = 4.0,
    UpperTorso = 1.0,
...
if not _G.PenetrationMetaTableInitialized then
    ...
    local ok, err = pcall(function()
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            and args[2][1] == (workspace:FindFirstChild("Debris") or nil) then
    ...
    if not ok then
        warn("⚠️ Penetration hooks not supported on this executor: " .. tostring(err))
    end
end
