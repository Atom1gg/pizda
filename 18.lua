...
local function createToggle(parent, setting, position)
    ...
    if shouldHaveBind then
        ...
        -- УВЕЛИЧИВАЕМ РАЗМЕР: ширина 80 вместо 60, высота 25 вместо 20
        keybindButton.Size = UDim2.new(0, 80, 0, 25)
        -- Сдвигаем позицию из-за увеличенного размера
        keybindButton.Position = UDim2.new(0.6, 290, 0.5, -12)
        ...
        -- УБИРАЕМ ЦВЕТА: всегда серый текст
        ...
        keybindButton.MouseButton1Click:Connect(function()
            keybindText.Text = "[...]"
            -- УБИРАЕМ ЦВЕТ ПРИ ОЖИДАНИИ: оставляем серый
            keybindText.TextColor3 = Color3.fromRGB(150, 150, 150)
            ...
            keybindSystem.listeningCallback = function(keyName)
                if keyName == "None" then
                    ...
                    -- УБИРАЕМ ЦВЕТ ПРИ АКТИВНОМ БИНДЕ: оставляем серый
                    keybindText.TextColor3 = Color3.fromRGB(150, 150, 150)
                end
            end
        end)
